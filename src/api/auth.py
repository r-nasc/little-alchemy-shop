from fastapi import Security, HTTPException, Request
from fastapi.security.api_key import APIKeyHeader
import os
import dotenv

dotenv.load_dotenv()

api_keys = [os.environ.get("API_KEY")]
api_keys.append(os.environ.get("API_KEY"))
api_key_header = APIKeyHeader(name="access_token", auto_error=False)


# pylint: disable=redefined-outer-name
async def get_api_key(request: Request, api_key_header: str = Security(api_key_header)):
    """Returns API Key passed in header if valid or raises an exception"""
    is_local = os.environ.get("IS_LOCAL")
    authorized_key = os.environ.get("API_KEY")

    if not (is_local or api_key_header == authorized_key):
        raise HTTPException(status_code=401, detail="Forbidden")

    request.state.is_demo = is_local
    return api_key_header
